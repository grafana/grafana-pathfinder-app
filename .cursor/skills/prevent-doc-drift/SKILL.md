...
- **Review mode** (invoked from `/review`): output a single fenced patch block per file with the proposed edits. Do not modify the working tree. `/review` turns that output into one non-blocking finding in its rendered report.
...
When `/review` runs, after the routed reviewers have produced their findings, invoke this skill in **review mode** to produce a "Doc-drift updates recommended" section. `/review` converts its concrete action into a non-blocking finding rather than pasting the section in verbatim — its report renderer owns the final output, and `.cursor/skills/review/SKILL.md` §7 owns which disposition it carries — and stays silent when this skill finds no drift. The PR author can then apply the diffs themselves or invoke this skill in apply mode to commit them.
