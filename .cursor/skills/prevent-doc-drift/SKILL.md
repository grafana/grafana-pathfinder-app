...
| New plugin stream message type                      | `.cursor/rules/systemPatterns.mdc`                      | Add to the "Stream message types" bullet under "Backend architecture (`pkg/`)". Also flag the code-side mirror: the frontend rejects any type missing from the Zod union in `src/integrations/coda/terminal-protocol.schema.ts`.                                                                                                                                                                                                                      |
