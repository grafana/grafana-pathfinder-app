 ...
 * 3. **Grafana Cloud**: Uses an opaque Bearer credential scoped to the target origin
 ...
 * ```typescript
 * const bearerTokenStrategy: AuthStrategy = {
 *   name: 'scoped-bearer-token',
 *   async authenticate(page, grafanaUrl) {
 *     // Bearer credentials use request headers, not interactive login
 *     return true;
 *   },
 *   async validateSession(page) {
 *     const response = await page.request.get('/api/user', {
...
export const pluginE2EAuthStrategy: AuthStrategy = {
  ...
  async refreshSession(page: Page, grafanaUrl: string): Promise<boolean> {
    ...
    //
    // For long-running tests, use a strategy that supports credential refresh.
