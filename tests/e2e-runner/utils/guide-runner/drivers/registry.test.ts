...
describe('STEP_DRIVERS', () => {
  ...
  it('supports plain, multistep, guided, terminal, terminal-connect, and codeblock behavior', () => {
    ...
    expect(supported).toEqual(['plain', 'multistep', 'guided', 'terminal', 'terminal-connect', 'codeblock']);
    expect(unsupported).toEqual(['quiz', 'challenge', 'datasource-check']);
  });
});
