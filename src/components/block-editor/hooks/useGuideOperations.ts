...
export interface UseGuideOperationsReturn {
  ...
  /** Replace the editor with a guide generated by the assistant */
  handleGenerateGuide: (guide: JsonGuide) => void;
...
export function useGuideOperations(options: UseGuideOperationsOptions): UseGuideOperationsReturn {
  ...
  // Replace the editor with an assistant-generated guide
  const handleGenerateGuide = useCallback(
    (guide: JsonGuide) => {
      editor.loadGuide(guide);
      onNewGuide?.();
      modals.close('generateGuide');
    },
    [editor, modals, onNewGuide]
  );
