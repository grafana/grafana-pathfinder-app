...
function validateGuide(data, options = {}) {
    ...
    if (!result.success) {
        return { isValid: false, errors: (0, errors_1.formatZodErrors)(result.error.issues, data), warnings: [], guide: null };
    }
...
function toLegacyResult(result) {
    return { isValid: result.isValid, errors: (0, errors_1.formatErrorsAsStrings)(result.errors), warnings: (0, errors_1.formatWarningsAsStrings)(result.warnings), guide: result.guide };
}
