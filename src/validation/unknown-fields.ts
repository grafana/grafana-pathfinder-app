...
function checkUnknownFields(obj: Record<string, unknown>, knownFields: ReadonlySet<string>, path: (string | number)[]): ValidationWarning[] {
...
function detectUnknownFieldsInBlock(block: Record<string, unknown>, path: (string | number)[]): ValidationWarning[] {
  const warnings: ValidationWarning[] = [];
  const blockType = block.type as string;
  const knownFields = KNOWN_FIELDS[blockType];
  if (!knownFields) return warnings;
  warnings.push(...checkUnknownFields(block, knownFields, path));
  if (Array.isArray(block.blocks)) {
    (block.blocks as Record<string, unknown>[]).forEach((nestedBlock, index) => {
  ...
  if (Array.isArray(block.steps)) {
    ...
    if (stepFields) {
      (block.steps as Record<string, unknown>[]).forEach((step, index) => {
  ...
  if (Array.isArray(block.choices)) {
    ...
    if (choiceFields) {
      (block.choices as Record<string, unknown>[]).forEach((choice, index) => {
...
export function detectUnknownFields(guide: unknown): ValidationWarning[] {
  ...
  if (guideFields) warnings.push(...checkUnknownFields(guideObj, guideFields, []));
  if (guideObj.match && typeof guideObj.match === 'object') {
    const matchFields = KNOWN_FIELDS._match;
    if (matchFields) warnings.push(...checkUnknownFields(guideObj.match as Record<string, unknown>, matchFields, ['match']));
  }
  if (Array.isArray(guideObj.blocks)) {
    (guideObj.blocks as Record<string, unknown>[]).forEach((block, index) => {
