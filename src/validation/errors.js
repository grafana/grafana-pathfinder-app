...
exports.formatErrorsAsStrings = formatErrorsAsStrings;
exports.formatWarningsAsStrings = formatWarningsAsStrings;
const json_guide_schema_1 = require("../types/json-guide.schema");
function getBlockTypeFromPath(data, path) {
    let current = data;
...
function getBlockAtPath(data, path) {
    let current = data;
    for (const segment of path) {
        if (current === null || current === undefined)
            return null;
...
function formatBlockPath(path, data) {
    const parts = [];
        ...
        if (path[i - 1] === 'blocks' && typeof segment === 'number') {
            ...
            for (let j = i - 2; j >= 0; j--) {
                if (path[j - 1] === 'blocks' && typeof path[j] === 'number') {
                    parts.push(`Block ${path[j] + 1}, step ${segment + 1}`);
                    break;
                }
            }
        }
...
function isFieldValidForBlockType(fieldName, blockType) {
    const knownFields = json_guide_schema_1.KNOWN_FIELDS[blockType];
    return knownFields ? knownFields.has(fieldName) : false;
}
function validateStepFields(step, stepIndex, blockIndex) {
    if (!step.action)
...
function validateNestedBlock(nestedBlock, nestedIndex, parentPath) {
    ...
    if (!json_guide_schema_1.VALID_BLOCK_TYPES.has(blockType))
...
function getUnionErrorMessage(issue, data) {
    ...
    if (blockType && !json_guide_schema_1.VALID_BLOCK_TYPES.has(blockType))
    ...
    if ((blockType === 'multistep' || blockType === 'guided') && Array.isArray(block.steps)) {
        const blockIndex = issue.path.length >= 2 && typeof issue.path[issue.path.length - 1] === 'number'
            ? issue.path[issue.path.length - 1] : 0;
    ...
    if ((blockType === 'section' || blockType === 'assistant') && Array.isArray(block.blocks)) {
        ...
        for (let i = 0; i < block.blocks.length; i++) {
            const nestedBlock = block.blocks[i];
    ...
    if (blockType === 'interactive') {
        const validActions = ['highlight', 'button', 'formfill', 'navigate', 'hover'];
        if (block.action && !validActions.includes(block.action)) {
...
function formatZodErrors(issues, data) {
        ...
        if (issue.code === 'invalid_union') {
            ...
            if (unionMsg) {
                ...
                if (unionMsg.startsWith('Block ')) {
                    ...
                    const blockIndex = issue.path[issue.path.length - 1];
            ...
            if (issue.code === 'too_small' || (issue.code === 'invalid_type' && 'received' in issue && issue.received === 'undefined')) {
                ...
                message = `${blockPath}: unknown ${fieldName} '${issue.received}'`;
            }
            else {
            ...
            if (issue.code === 'too_small') {
                ...
                message = `Guide is missing required field '${fieldName}' (${issue.expected})`;
            }
...
function formatErrorsAsStrings(errors) {
    return errors.map(e => e.message);
}
function formatWarningsAsStrings(warnings) {
