...
it('reuses the original key and expiry after a lost response, even if polling sees the new grant', async () => {
  ...
  const { rerender } = render(
    <SandboxLifetime client={api as unknown as typeof lifetimeClient} vmId="vm-test" vm={vm} />
  );
  fireEvent.click(await screen.findByRole('button', { name: 'Extend by 30 minutes' }));
  rerender(<SandboxLifetime client={api as unknown as typeof lifetimeClient} vmId="vm-test" vm={extended} />);
...
it('hides the control after all three extensions', async () => {
  ...
  render(<SandboxLifetime client={api as unknown as typeof lifetimeClient} vmId="vm-test" vm={await api.getVM()} />);
...
it('hides extension controls when the upstream has no lifetime contract', async () => {
  ...
  const { container } = render(
    <SandboxLifetime client={api as unknown as typeof lifetimeClient} vmId="vm-test" vm={await api.getVM()} />
  );
  await waitFor(() => expect(api.getVM).toHaveBeenCalled());
