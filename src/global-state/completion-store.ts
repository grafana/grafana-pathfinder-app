...
export function markStepCompleted(stepId: string, sectionId: string | undefined, reason: ProgressReason): void {
  ...
  // `'skipped'` is sticky until a reset clears the entry. A skipping step
  // writes it through the checker's bridge, and its section then reports the
  // same step complete with the generic `'manual'` — which would otherwise
  // relabel a skipped check as one the user actually passed.
