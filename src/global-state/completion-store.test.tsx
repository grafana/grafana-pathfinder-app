...
describe('completion-store', () => {
  ...
  it('does not relabel a skipped step as one the user completed', async () => {
    // A skipping step writes `'skipped'` through the checker's bridge, and
    // its section then reports the same step complete with the generic
    // `'manual'`. Taking that second write would tell the user their
    // skipped check had actually passed.
    render(<StepProbe stepId="step-1" sectionId="section-x" />);
    await flushMicrotasks();
    act(() => markStepCompleted('step-1', 'section-x', 'skipped'));
