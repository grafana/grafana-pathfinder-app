...
function isAutoGeneratedId(id: string): boolean {
  ...
  // React's useId() (e.g. ":r15p:") is sanitized to underscores for use as an HTML id
  // attribute (e.g. "_r_15p_", or "start-panel-_r_15p_" when a component prefixes it).
  // It's tied to render-tree position, not to anything semantic, so it's no more
  // stable than the colon form already covered by the session-unstable flag below.
  if (/_r_[0-9a-z]+_$/i.test(id)) {
    return true;
  }
  return false;
}
...
/**
 * Bare tag name with no class/text/position baked in — relies entirely on
 * `disambiguate()` walking out to the nearest ancestor scope where it's unique.
 * Scored well below text/aria candidates so it wins when a clean, meaningful
 * testid scope makes a plain tag unique (e.g. `[data-testid='X'] button`),
 * instead of falling back to i18n-sensitive text matching or a hashed class.
 */
function candidateBareTag(element: HTMLElement): Candidate | null {
  if (!element.parentElement) {
    return null;
  }
  const tag = element.tagName.toLowerCase();
  return { selector: tag, score: CANDIDATE_SCORES.scopedBareTag, method: 'scoped-bare-tag' };
}
...
function disambiguate(
  ...
  for (const scope of ancestorScopes) {
    ...
    // Skip for scoped-bare-tag: a bare tag name only earns its low score when a
    // stable ancestor makes it unique on its own (1a). Falling back to a positional
    // nth-child index is really the same fragility as the dedicated nth-of-type
    // candidate, just wearing a much better score it didn't earn — so treat it as
    // "no match" here and let nth-of-type (scored honestly at 10000+) compete instead.
    const branch = candidate.method === 'scoped-bare-tag' ? null : findDirectChildBranch(scope.element, element);
    if (branch) {
  ...
  // Strategy 3: nth-match (last resort) — same rationale as 1b above, skip for scoped-bare-tag.
  if (results.length === 0 && candidate.method !== 'scoped-bare-tag') {
    try {
      const matchFn = inOverlay ? querySelectorAllEnhancedVisible : querySelectorAllEnhanced;
