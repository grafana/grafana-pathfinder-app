...
/**
 * Extract the stable prefix from an id that has a dynamic suffix (e.g. Grafana
 * React 18's `:rXX:` auto-id). Returns null if no stable prefix survives or if
 * the prefix itself looks auto-generated (uuid segment, hash chunk, etc.).
 */
function extractStableIdPrefix(id: string): string | null {
  if (!id) {
    return null;
  }

  // React 18 / Grafana useId pattern — `foo-:r9j:` or `foo:ra6:bar`.
  // Only strip this suffix; other dynamic-looking ids should be treated as a
  // whole, not split into speculative prefixes.
  const match = id.match(/^(.*?)[-_]?:[a-z0-9]+:.*$/i);
  if (!match || !match[1]) {
    return null;
  }
  ...
  // Reject prefixes that themselves look auto-generated.
  if (/^[a-f0-9]{6,}$/i.test(prefix)) {
    return null;
  }
  if (/^[0-9a-f]{8}-[0-9a-f]{4}/i.test(prefix)) {
    return null;
  }
  return prefix;
}
...
/**
 * True when a form-control `name` attribute looks stable enough to use as a
 * selector. Radio/checkbox inputs inside React 18 forms often have dynamic
 * names like `:ra6:` — skip those, but allow plain kebab/snake/camel tokens.
 */
function isStableFormName(name: string): boolean {
  if (!name) {
    return false;
  }
...
/**
 * For elements whose full id is rejected as auto-generated (e.g. Grafana React
 * 18's `option-builder-:ra6:` pattern), fall back to an attribute-prefix
 * selector that targets the stable portion.
 */
function candidateIdPrefix(element: HTMLElement): Candidate | null {
  if (!element.id) {
    return null;
  }
  const prefix = extractStableIdPrefix(element.id);
  if (!prefix) {
    return null;
  }
  const tag = element.tagName.toLowerCase();
  return {
    selector: `${tag}[id^='${prefix}']`,
    score: CANDIDATE_SCORES.idPrefix,
    method: 'id-prefix',
  };
}

/**
 * For `<input>` elements with a short, stable `value` attribute (radios,
 * checkboxes, hidden inputs). The value is a strong discriminator between
 * siblings in a radio group where `data-testid` is shared.
 */
function candidateInputValue(element: HTMLElement): Candidate | null {
  if (element.tagName !== 'INPUT') {
    return null;
  }
  const value = element.getAttribute('value');
  if (!value || value.length === 0 || value.length > SELECTOR_CONFIG.maxTextLength) {
    return null;
  }
  // Skip values that look like free-form user input or timestamps.
  if (value.includes('\n') || value.includes(' ')) {
    return null;
  }
  if (/^\d{6,}$/.test(value)) {
  ...
  const stableType = inputType && /^(radio|checkbox|hidden|submit|button|reset)$/.test(inputType);

  if (stableType) {
    return {
      selector: `input[type='${inputType}'][value='${value}']`,
      score: CANDIDATE_SCORES.inputValue,
      method: 'input-value',
    };
  }
  return {
    selector: `input[value='${value}']`,
    score: CANDIDATE_SCORES.inputValue + 10,
    method: 'input-value',
  };
}

/**
 * Find a visible label that identifies this form control. Check four sources:
 *   1. `<label for="id">` pointing at this element (by full id or stable prefix).
 *   2. Ancestor `<label>` wrapping the control.
 *   3. `aria-labelledby` target's text content.
 *   4. An adjacent `<label>` sibling.
 *
 * Returns the label text and an optional HTMLElement when the text is backed
 * by an adjacent-sibling label that we can target with `+`.
 */
function findAssociatedLabelText(element: HTMLElement): { text: string; adjacentLabel?: HTMLLabelElement } | null {
  const tag = element.tagName.toLowerCase();
  if (!['input', 'select', 'textarea'].includes(tag)) {
    return null;
  }

  const toText = (el: Element | null): string => normalizeText(el?.textContent || '');

  // 1. label[for=ID]
  if (element.id) {
    try {
      const escapedId = element.id.replace(/['"\\]/g, '\\$&');
      const match = element.ownerDocument?.querySelector<HTMLLabelElement>(`label[for='${escapedId}']`);
      if (match) {
        const text = toText(match);
        if (text) {
          return { text, adjacentLabel: match.parentElement === element.parentElement ? match : undefined };
        }
      }
    } catch {
      /* invalid id for query */
    }
  }
  ...
  while (ancestor && depth < 4) {
    if (ancestor.tagName === 'LABEL') {
      const text = normalizeText((ancestor.textContent || '').trim());
      if (text) {
        return { text };
      }
    }
  ...
  if (labelledBy) {
    ...
    for (const id of ids) {
      try {
        ...
        const target = element.ownerDocument?.getElementById(escaped);
        if (target) {
          parts.push(toText(target));
        }
      } catch {
        /* ignore */
      }
  ...
  // 4. adjacent sibling label (previous / next / anywhere in parent)
  ...
  if (parent) {
    ...
    for (const sibling of siblings) {
      ...
      if (sibling.tagName === 'LABEL') {
        const text = toText(sibling);
        if (text) {
          return { text, adjacentLabel: sibling as HTMLLabelElement };
        }
      }
    }
  }
...
/**
 * Target form controls via their associated label text. Useful when the
 * control itself has no stable attributes but sits next to a `<label>Builder</label>`.
 */
function candidateSiblingLabelText(element: HTMLElement): Candidate | null {
  const tag = element.tagName.toLowerCase();
  if (!['input', 'select', 'textarea'].includes(tag)) {
    return null;
  }

  const result = findAssociatedLabelText(element);
  if (!result) {
    return null;
  }

  const labelText = result.text.slice(0, SELECTOR_CONFIG.maxTextLength);
  const safeText = labelText.replace(/'/g, "\\'");

  if (result.adjacentLabel) {
    // Adjacent sibling — use sibling combinator. Prefer `+` when the label is
    // immediately previous, otherwise `~`.
    const parent = result.adjacentLabel.parentElement;
    if (parent) {
      const children = Array.from(parent.children);
      const labelIdx = children.indexOf(result.adjacentLabel);
      const targetIdx = children.indexOf(element);
      const combinator = targetIdx === labelIdx + 1 ? '+' : '~';
      return {
        selector: `label:contains('${safeText}') ${combinator} ${tag}`,
        score: CANDIDATE_SCORES.siblingLabel,
        method: 'sibling-label',
      };
    }
  }

  // No adjacency — fall back to a generic "within a group that contains this
  // label" selector. The disambiguation pass will scope it further if needed.
  return {
    selector: `:has(> label:contains('${safeText}')) ${tag}`,
    score: CANDIDATE_SCORES.siblingLabel + 10,
    method: 'sibling-label',
  };
}
...
function candidateName(element: HTMLElement): Candidate | null {
  ...
  if (!isStableFormName(name)) {
    return null;
  }
  if (tag === 'input') {
    const inputType = (element as HTMLInputElement).type?.toLowerCase();
    // For radio/checkbox the name refers to the group, so the selector is not
    // unique to the specific option. Only emit it when we can also anchor on
    // a type — the value candidate handles per-option targeting.
    if (inputType === 'radio' || inputType === 'checkbox') {
      return {
        selector: `${tag}[type='${inputType}'][name='${name}']`,
        score: CANDIDATE_SCORES.name + 10,
        method: 'name',
      };
    }
  }
...
function candidateCompound(element: HTMLElement): Candidate | null {
  ...
  if (isFormControl) {
    // For form controls, stable classes are rare and meaningless — prefer
    // type + value, or name, before falling back to classes.
    const inputType = tag === 'input' ? (element as HTMLInputElement).type?.toLowerCase() : null;
    if (inputType) {
      parts.push(`[type='${inputType}']`);
    }
    const value = element.getAttribute('value');
    if (value && value.length > 0 && value.length <= SELECTOR_CONFIG.maxTextLength && !/\s/.test(value)) {
      parts.push(`[value='${value}']`);
    }
    const name = element.getAttribute('name');
    if (name && isStableFormName(name) && !parts.some((p) => p.includes("value='"))) {
