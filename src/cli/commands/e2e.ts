...
export const e2eCommand = new Command('e2e')
  ...
  .action(async (files: string[], options: E2ECommandOptions) => {
    ...
    try {
      ...
      const idsToSkip = idsSkippedForUnsafeSharedStack(plan, inputs.packageMetaById, inputs.cloudStack);
      const idsUsingColdStack = idsUsingColdCloudStack(
        plan,
      ...
      await runPreflightChecks(
        options,
        targetUrlsToCheck(inputs, options.grafanaUrl, new Set([...idsToSkip, ...idsUsingColdStack])),
