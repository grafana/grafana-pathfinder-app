 ...
 * Contract: mcp-native
 *
 * Reuses the canonical `runValidate` runner, but the input is an in-memory
 * artifact (or session token), not the disk-oriented CLI `validate` interface.
 * Explicit top-level Zod is authoritative; agents do not use pathfinder_help
 * or an `opts` bag.
 */
