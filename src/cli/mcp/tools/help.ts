...
export function registerHelpTool(server: McpServer): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Use this tool when you need the exact parameter interface for a Pathfinder CLI-backed tool while authoring a guide. What comes back is the command's own schema rendered as JSON — not CLI syntax, so you never build a command line. Each parameter arrives under its camelCase name with schema-owned descriptions, types, and requiredness. Copy those names and JSON value types directly into `opts`: booleans are `true`/`false`, repeatables are arrays, enums are strings. Positional CLI arguments are republished as ordinary named parameters. Parameters arrive in three lists — `required`, `optional`, and `addressing` — and `addressing` holds both required and optional ones, so read `requiredParams` for what you must send and the union of all three for what you may send; every parameter also states its own `required` flag. When the response lists `subcommands`, pass your chosen value in `opts` and call this tool again with that value as `subcommand` for the full per-subcommand surface; a subcommand's own `requiredParams` is the parameter that selects it followed by the root response's `requiredByType` entry for it, so `requiredByType` alone is that list minus the selector. A `requirementTokens` array, when present, is the complete vocabulary for that command's `requirements` / `conditions` parameters — nothing outside it validates. Parameters a Pathfinder tool already takes as its own argument are omitted and rejected if sent. For pathfinder_manage_block and pathfinder_manage_guide, `command` is the same value as `operation`. Omit command for the command list.",
