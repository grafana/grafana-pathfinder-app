...
| `EXTERNAL_API.md`                      | External guide-import API (`POST /v1/guides/import`) — endpoint, auth, RBAC, request/response, error codes, curl examples. Read when working on `pkg/plugin/guides_*.go` or the import contract.                                 | `pkg/plugin/guides_*.go`                                                                          |
