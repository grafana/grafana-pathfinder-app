...
Routes live in `resources.go`; the per-feature proxies are `completion_records.go`, `custom_guide_repository.go`, and `package_recommendations.go`, sharing `app_platform_client.go` (paginated LIST) and `app_platform_identity.go` (the shared identity gate, over `auth/id_token.go`). Plugin entrypoint is `pkg/main.go`.
