...
The concern registry, reviewer fleet, and routing tables in `CONCERNS.md` + `.cursor/rules/pr-review.md` are operationally sound but invocation is by agent cooperation. Husky pre-commit runs per-file lint/format checks through `lint-staged`, but it does not trigger `/review`. There is no `Stop` hook, PR-creation hook, or CI gate that confirms `/review` ran on a diff before merge.
...
`src/components/docs-panel/docs-panel.tsx` was 2,681 LOC with 42 imports at audit time; it is 1,464 LOC with 40 imports today, after the #894 renderer extraction and the pure-seam extraction that followed. It still spans Tier 0–3 in one file — engines (`interactive-engine`, `learning-paths`, `package-engine`), support (`lib/`, `recovery/`, `styles/`, `utils/`, `global-state/panel-mode`), integrations (`coda`, `workshop`) — and lazy-loads three heavy components. It is no longer the largest component in `src/components/`, but it remains the panel's coordination hub, and the tier model has nothing to say about it because all edges point the right direction. The tier ratchet enforces _direction_ of dependencies; it does not enforce _concentration_.
...
| B7  | Tighten husky pre-commit            | F-9            | Implemented by #934, then superseded by #1823 after `typecheck` + `test:governance` measured ~52s combined. Current pre-commit runs `lint-staged`; whole-project checks stay in `npm run check` and CI.                                                                      | Fast per-file checks run at commit time; whole-project gates remain enforced before merge.                                                                      | —        | A2, B5     |
...
Cover the agent-system threat surface as framed by OWASP Agentic Top 10 and the NIST AI Agent Standards Initiative (2026). Specific attack surface for this repo: MCP tools (TS server now, post-#888), `WebFetch`-loaded refactor guidelines, sandbox terminal access (now owned by `grafana-coda-app`, consumed here through `src/integrations/coda/`), agent-authored code reaching a customer-facing plugin.

| id  | item                                    | problem (cite / motivation)                                                                                                                                                       | proposed change                                                                                                                                                                                                                                                                                                                                                                                                                       | acceptance criteria                                                                                                                                             | effort   | depends on |
| --- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------- |
| D1  | Agent threat model document             | No enumerated threat model for agent operation today.                                                                                                                             | Create `docs/design/AGENT_THREATS.md`. Enumerate: tool misuse (MCP tool used destructively); context poisoning (a fetched guide telling the agent to do X); goal hijack (a PR description prompting the reviewer); credential/privilege abuse (the App Platform proxies in `pkg/`, and sandbox exec through the Coda plugin); supply-chain (skill drift, npm); cascading failure across multi-agent chains. Map each to a mitigation. | Document exists, is referenced from `AGENTS.md` "PR reviews" section, and each threat has at least one named mitigation (existing or planned).                  | 1–2 days | —          |
...
| D4  | Terminal/SSH session audit log          | Sandbox terminal access now belongs to `grafana-coda-app` (#1468 deleted `pkg/plugin/stream.go` and `terminal.go`), so this item moves to that repo. OWASP "immutable tool logs". | Raise with the Coda team: audit logging of session lifecycle (open/close/command-executed) to a tamper-evident sink. Nothing to build here — Pathfinder holds no session state and issues exec through the SDK.                                                                                                                                                                                                                       | Every terminal session emits open/close events with session id; every executed command is logged at least at the metadata level.                                | 1–2 days | D1         |
...
| E3  | Decompose `docs-panel.tsx`                    | Partially discharged: #894 extracted the renderer, and a follow-up extracted the pure tab-state and docs-load policy seams, taking the file from 2,681 LOC / 42 imports to 1,464 LOC / 40 imports. Remaining, using the `interactive-section` template: auto-launch wiring, panel-mode coordination, workshop glue, and the async `loadTabContent` state machine. | File size and import count drop below the ratchet allowlist entries set in B3. The ratchet allowlist shrinks accordingly. | 2–3 days | B3         |
