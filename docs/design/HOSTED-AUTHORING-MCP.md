...
| `pathfinder_help`                      | Translates a bound CLI command/subcommand's help into the camelCase parameter interface imported runners receive          |
...
`pathfinder_manage_guide` with `operation: "set-manifest"` is included in the MVP tool surface so AI authoring produces correctly-shaped manifest data inside the artifact. **That manifest data is projected onto the App Platform CRD at `spec.manifest`** — the `InteractiveGuide` resource types a subset of the keys verbatim and sweeps the remainder into `additionalFields`, its escape hatch, so nothing authored is lost on the way in. This applies to all custom guides (block-editor and AI alike), not just AI authoring; see [Grafana App Platform publish handoff — Manifest fields at publish](./APP-PLATFORM-PUBLISH-HANDOFF.md#manifest-fields-at-publish).
...
- **`pathfinder_help` is the discovery surface.** Agents call it with a bound CLI command name (and optional subcommand) to get the parameter-level contract for a CLI-backed payload; calling it with no argument lists exactly the bound set. For `pathfinder_manage_block` and `pathfinder_manage_guide`, pass `command` equal to `operation`; for `add-block`, choose a listed subcommand and pass that value as `opts.type`. The published interface is rendered from the command's own Zod schema, so parameter names, value types, requiredness, and roles are read rather than inferred, and there is nothing for a binding to translate. Agent-facing prose about a parameter is the field's `.describe()` text, which is also what CLI users read in `--help`. New CLI capability on a bound command therefore reaches agents without an adapter change: the schema that publishes a parameter is the schema that parses it. **Binding itself is opt-in:** only a command an MCP tool has registered is addressable, so a CLI command no tool dispatches (`e2e`, `build-graph`, `move-block`, `rename-id`, `validate`, …) reports `UNKNOWN_COMMAND` and is absent from the command list. Describing its flags would only have an agent author against a tool that does not exist — and for `move-block` / `rename-id` specifically, against capability the append-only procedure excludes on purpose. What an agent may _not_ set is the binding's own statement rather than the command's: `bindCommandInterface('add-block', { withhold: ['before', 'after', 'position'] })` narrows the agent procedure without the command knowing this surface exists, and every withheld name is checked against the schema at boot, so a renamed parameter fails loudly instead of quietly publishing itself.
- **1:1 CLI tools use the same bag.** `pathfinder_create_package`, `pathfinder_inspect`, and `pathfinder_get_schema` place their CLI-owned parameters in `opts`; MCP transport/orchestration values such as `artifact` and `sessionToken` remain top-level. Each binding forwards its bag to the command's schema unchanged, leaving the CLI's Commander declaration untouched.
...
- **The adapter answers the questions Commander's parser would.** MCP reaches the runners without going through the parser, so `validateCommandArgs` reports a missing required parameter, an unknown one, or one withheld from agents before the runner is called. It reads the command's schema — the same schema the runner is parsed against — so the preflight exists to name the problem in the agent's vocabulary, not to hold a second opinion about the shape. Requiredness is enforced by the schema on both entrypoints, which is what lets a command like `add-block` report every missing field in one pass instead of Commander's first-failure.
- **Resolved: dispatch no longer names runner fields.** This was a known follow-up while discovery tracked the CLI but dispatch did not — bindings mapped keys onto imported `runX` argument structs (`parent` → `parentId`, `id` → `explicitId`, …), so a new CLI flag consumed as a named runner argument needed an MCP binding change. Two routes were considered and both rejected: in-process Commander dispatch, blocked by `src/cli/index.ts` parsing `process.argv` at import and every action calling `process.exit()`; and subprocess dispatch, which works but pays spawn latency. The route taken instead was to make the command's schema the authority both entrypoints render, so neither has to parse for the other — see [`CLI-MCP-COMMAND-CONTRACT`](../../../pathfinder-rfcs/rfc/CLI-MCP-COMMAND-CONTRACT.md). A binding now forwards its bag to `parseCommandInput(spec, bag)` and runs the result.
