...
| `pathfinder_manage_block`              | Add, edit, or remove blocks, or append steps/choices/hints, via a CLI-named `operation` plus an opaque, help-derived `opts` bag |
...
```json
{
  ...
  "workflow": [
    ...
    "2. Mutate the tree via pathfinder_manage_block with operation \"add-block\" | \"edit-block\" | \"remove-block\" | \"add-step\" | \"add-choice\" | \"add-hint\" (CLI command names). Set guide metadata via pathfinder_manage_guide with operation \"set-manifest\". …",
