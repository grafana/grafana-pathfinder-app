...
Signal counting: a matching changed file path = 1 signal; a high-value semantic hit (core symbol, API, state key, or contract name) = 1 signal; repeated low-value hits in the same hunk = 1 signal total. Never activate concerns using ordinary trigger paths alone when semantic evidence is absent, unless the concern is always-on. A match in the contract owner routing table is itself a high-value semantic signal and can activate its concern without a keyword hit.

### Contract owner routing

Use owner paths for narrow files or directories that define a subsystem contract. Do not grow keyword lists to enumerate every symbol owned there.

<!-- prettier-ignore -->
| concern | owner_paths |
| --- | --- |
| `cli-and-e2e-runner` | `src/cli/contracts/**` |
| `mcp-authoring-server` | `src/cli/contracts/**` |
