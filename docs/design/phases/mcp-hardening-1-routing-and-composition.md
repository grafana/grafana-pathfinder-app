...
- [ ] **2. M1 layer 3 — server `instructions` wiring.** New module `src/cli/mcp/lib/server-instructions.ts` exporting a constant string with: (a) trigger vocabulary (when to use this server), (b) one-line "never invent DOM selectors" rule, (c) one-line "prefer sibling blocks over multistep, never write noop steps as filler" rule. Pass via `new McpServer(..., { capabilities, instructions: SERVER_INSTRUCTIONS })` in `buildServer` (`src/cli/mcp/server.ts:32`). Unit test asserts non-empty `instructions` is included in `initialize` response (use existing MCP test harness pattern).
