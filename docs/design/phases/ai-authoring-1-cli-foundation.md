...
- **Distribution path for P2.** The existing `npm run build:cli` produces the dist artifact at `dist/cli/cli/index.js`. P2 wraps the package for npm publish and Docker — no Node SEA, no `pkg`, no plugin-tarball coupling. P3 adds the `mcp` subcommand to the same `pathfinder-cli` binary.
