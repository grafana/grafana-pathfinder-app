...
- **OQ2 (step / choice block ids) is _deferred pending telemetry_** (decision 2026-05-12). The three options were scoped (required ids + migration; additive optional + auto-id on read; index-based addressing) but the right answer depends on how often agents actually mid-session-edit a step — speculation from a single observed session isn't enough. Re-open after slices 1+2 ship to production and a week or two of real Grafana Assistant traffic reveals the frequency. If cascade-rebuild is common, scope a slice around OQ2; if rare, the current state may be fine indefinitely. Future planners: check the issue #4 Status annotation in `MCP-AGENT-UX-HARDENING.md` before picking this up — it carries the full rationale.
