...
- Parameters are published in three lists, and only two of them are named for requiredness. `addressing` is named for role and overlaps both: `add-block --type section`'s `id` is required and lands there, so `required` + `optional` is **not** the interface. `requiredParams` states every parameter the caller must supply regardless of bucket — the same list `requiredByType` reports for a group variant, and the same list the preflight demands — and every parameter also carries its own `required` flag. A consumer that reads only the two requiredness-named buckets builds a field list with a required parameter missing from it.
