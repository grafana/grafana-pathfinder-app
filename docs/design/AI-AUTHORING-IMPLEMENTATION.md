...
| P7    | GCS-backed authoring sessions          | Complete    | [ai-authoring-7-gcs-sessions.md](./phases/ai-authoring-7-gcs-sessions.md)                 | _epic issue TBD_ |
...
- ~~**GCS-backed authoring sessions (deferred from P3).**~~ **Done — shipped as P7 (2026-05-20).** See [`phases/ai-authoring-7-gcs-sessions.md`](./phases/ai-authoring-7-gcs-sessions.md). Implementation matches the design captured below, with three executive deviations recorded in the P7 decision log: `_session` suffix on the session-scoped manifest read tool (P6 collision avoidance), `expectedGeneration` defaults to server-side retry-once, and `pathfinder_apply_ops` is not implemented. The bullet text is preserved below as a reference for the original deferred-work writeup; the canonical implementation contract is now P7's phase plan.

- **GCS-backed authoring sessions — original deferred-work writeup (2026-05-01, superseded by P7).** The pre-P7 stateless model passed the full `{content, manifest}` artifact in _and_ out of every mutation tool. Real multi-hop authoring runs on Cloud Run (2026-05-01) showed total wire bytes scaling roughly O(N²) in the number of hops — a 27-hop adversarial guide cost ~50× more agent-side tokens than a single-shot author of the same final artifact. Token cost was the visible problem; the deeper one was **agent confabulation** — when the artifact lived in the agent's context across hops, the agent occasionally edited it speculatively between mutations, producing extra validation roundtrips. Both are solved by storing the artifact server-side and removing it from the wire. This is **not** a package repository (see [Repository-ification deferred](#) below) — App Platform is the per-tenant package store; this bucket is ephemeral working storage for drafts.
