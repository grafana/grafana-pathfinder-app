...
| MH5   | Retire the Go MCP entirely                    | Not started           | [mcp-hardening-5-retire-go-mcp.md](./phases/mcp-hardening-5-retire-go-mcp.md)                           | Removes `launch_guide` + pending-launch queue — both architecturally obsolete under the central TS MCP + Assistant web-surface handover (P4) |
...
- ~~Migrate Go MCP runtime tools to the TS package~~ — **done.** The five stateless tools (`list_guides`, `get_guide`, `get_guide_schema`, `validate_guide_json`, `create_guide_template`) were migrated under [MH4](./phases/mcp-hardening-4-go-mcp-migration.md) (2026-05-15). The earlier framing here said `launch_guide` and the `pending-launch` queue would stay in Go indefinitely because they were coupled to per-instance frontend polling. That framing was reversed once the new architecture — a single centrally-hosted TS MCP on Cloud Run plus Grafana Assistant's web-surface tools for the handover (P4 status note, 2026-05-15) — made the per-instance back-channel unused. The full retirement of `pkg/plugin/mcp.go`, including `launch_guide`, the queue, and the frontend polling hook, is tracked under [MH5](./phases/mcp-hardening-5-retire-go-mcp.md).
