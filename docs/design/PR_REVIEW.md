...
**Rendered detail is capped; the ratchet's memory is not.** Every follow-up a round produces enters the marker's `deferred` list — gate demotions, §4b demotions, tech debt, doc drift, instrumentation, and §5's suggestion-surface rule alike — so none of them can lose the "never raised as a blocker on a later round" suppression that `deferred` grants. Only the _rendering_ is bounded: the first 20 follow-ups render in full, and any beyond that become one compact line naming their IDs and pointing at the round that described them, which is what keeps the body postable. A follow-up is never withheld from the record to make room for another; the renderer rejects only an outright runaway above 30, which no report obeying §5 reaches.
