...
- `carried_forward` — optional on a `follow_up`; `true` marks an entry carried over from the prior marker's `deferred` list. It decides who keeps the follow-up budget, not how the entry renders
...
**The follow-up budget yields to the carry-forward, never the reverse.** A report renders at most 20 follow-ups, and `carried_forward: true` entries claim that budget first; new follow-ups fill whatever remains and the rest are withheld, with the section stating how many. Withholding a new follow-up is safe — it is already gate-demoted — while dropping an unresolved carried entry would silently discard the suppression `deferred` grants it, which is the round-3-reverses-round-1 failure the marker exists to prevent. The two caps are one bound, not two: `deferred` holds at most 20 entries, so a carried-forward set can never exceed the follow-up budget, which is what keeps the rendered body postable. The renderer therefore never rejects a report for having too many follow-ups; it rejects only a carried-forward set above 20, which the marker cap makes unreachable.
