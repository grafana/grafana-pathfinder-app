...
**Rendered detail is capped; the ratchet's memory is not.** Every follow-up a round produces enters the marker's `deferred` list — gate demotions, §4b demotions, tech debt, doc drift, instrumentation, and §5's suggestion-surface rule alike — so none of them can lose the "never raised as a blocker on a later round" suppression that `deferred` grants. No follow-up count is rejected: the 20-entry render cap is a compression threshold, not a bound on the report.

**A new follow-up always renders in full; only a carried one compresses.** Its `proposed_issue` exists nowhere but this report, so compressing it would destroy the text the invoker files and leave the next round nothing to carry forward. A `carried_forward` entry has already been described by the round that raised it, so a line of identifiers pointing back there is a true statement rather than a dead pointer. Beyond twenty rendered entries the carried ones compress, oldest budget first; if new follow-ups alone exceed twenty they all still render, because there is no safe way to shorten them.
