...
- **Nothing at all happens after `setOverride` — no auto-open, no exposure event, no `Using local override` warning.** Check the `variant` spelling. Anything outside `excluded` / `control` / `treatment` rejects the whole override and falls back to the default config; see the variant table in [`FEATURE_FLAGS.md`](./FEATURE_FLAGS.md#pathfinderhighlighted-guide-experiment).
