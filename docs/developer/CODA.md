...
terminal/{vmId}/{nonce}/{template}/{app}          → custom template + app
```
...
1. **In-memory cache** — `userVMs` map (`userLogin → vmID`). Check if cached VM is usable and matches requested template+app.
2. **ListVMs fallback** — Query Coda API for user's active VMs. Match template+app.
3. **Create new** — Check quota (max 3 VMs), then `CreateVM`.

Template+app scoping: if the user's existing VM has a different app (e.g., `nginx` vs `mysql`), the old VM is destroyed and a new one is created. This ensures switching between sample apps gives a fresh environment.
...
- `connect(vmOpts?)` subscribes to `plugin/grafana-pathfinder-app/terminal/new/{nonce}/{template?}/{app?}`.
...
- Provides fields for description, button text, VM template, and app name.
...
- The `useSampleApps` hook handles the fetch with loading/error states.
