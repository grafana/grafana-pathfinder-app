...
| `TELEMETRY.md`             | Faro + RudderStack telemetry: what a new feature gets for free vs when to add custom facade ops, privacy invariants, gating. Load when touching telemetry code or instrumenting a feature.      | `src/lib/telemetry/*`, `src/lib/analytics.ts`, `src/lib/logging.ts` |
