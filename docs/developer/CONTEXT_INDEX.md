...
| `COMPLETION_METRICS.md`    | How completion is computed and reported for guides vs learning paths — the progress_step/progress_total/completion_percentage trio, milestone URL resolution, storage semantics, edge cases     | `src/lib/analytics.ts`, `src/global-state/completion-store.ts` |
