...
`clearAllForContent` (the per-guide reset) writes no record of its own — the interim scheme completed by writing a per-content marker, and a refused write left the old progress live and still read. It can still fail: the hybrid backend records each delete with a `__timestamp` companion, and browser storage can refuse that write. The guarantee is not that a reset cannot fail but that a failed one says so — the reset re-scans after deleting and throws if any record survived, instead of appearing to work.
