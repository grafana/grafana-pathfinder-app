...
`clearAllForContent` (the per-guide reset) deletes records and writes nothing, then re-scans and throws if any survive, so a reset that could not complete reports itself instead of appearing to work.
