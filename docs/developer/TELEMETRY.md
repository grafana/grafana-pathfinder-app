...
Session replay adds a second remote switch (`pathfinder.session-replay`, also default-on) but no new environment gate: it is registered from inside `initFaro`, after the `resolveFaroEnvironment()` early return, so a self-hosted or OSS Grafana never reaches it — such an instance does not construct a Faro instance in the first place, and the rrweb chunk is never fetched. The open that latches the activity gate is also what starts the recording, and once started it runs for the rest of the page, including after Pathfinder is closed again.

Two consequences of it being default-on: recordings are only viewable on a stack where Grafana has switched on the private-preview feature, and Grafana core ships its own replay recorder behind `FlagKeys.FaroSessionReplay` — nothing stops both from running on the same page, so `pathfinder.session-replay` must be set false wherever core's flag goes true.
