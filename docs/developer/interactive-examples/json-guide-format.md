...
While the flagged pilot is active, each outcome supplies bounded, read-only page context to Grafana Assistant: the guide revision, outcome declaration, selected resource UID, latest verdict, and historical verification time. Failed-check context uses fixed check names and verdicts, not raw errors or commands. Registrations clear on guide, revision, or identity changes and unmount. Assistant has no evidence-writing tool; only the deterministic outcome checker can record a successful check.
